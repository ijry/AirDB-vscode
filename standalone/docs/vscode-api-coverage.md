...
| Text editor lifecycle | `window.activeTextEditor`, `window.onDidChangeActiveTextEditor`, `window.onDidChangeTextEditorSelection`, `workspace.onDidChangeTextDocument` | Host-owned editor sessions provide stable editor ids, active-editor events, selection events, and a document-model change event contract. Workbench content is still read-only: no editable buffer, `TextEditor.edit` apply path, `workspace.applyEdit`, save lifecycle, decoration rendering, or multi-selection UI parity yet. |
