...
| Area | APIs | Gap |
| --- | --- | --- |
| Configuration | `workspace.getConfiguration` | Reads default values only; no persistent configuration, section updates, or change events yet. |
| Extensions | `extensions.getExtension`, `extensions.all` | Returns extension metadata, but activated exports are not shared across loaded extensions yet. |
...
| Area | APIs |
| --- | --- |
| Workspace watchers | `workspace.createFileSystemWatcher` |
| Configuration events | `workspace.onDidChangeConfiguration` |
