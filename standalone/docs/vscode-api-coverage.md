...
| Text documents | `workspace.openTextDocument`, `window.showTextDocument` | Supports local files, untitled content, stable editor-session DTOs, and read-only workbench projection. |
...
| Languages | Basic provider registration for completions, code lens, hover, range formatting, and document symbols | Provides value types, registration storage, and standalone IPC invocation for the core provider paths; full VS Code editor lifecycle parity is not claimed. |
...
| Area | APIs | Gap |
| --- | --- | --- |
| Language providers | `languages.registerCompletionItemProvider`, `registerHoverProvider`, `registerDocumentSymbolProvider`, `registerDocumentRangeFormattingEditProvider`, `registerCodeLensProvider` | Completion, hover, CodeLens, document symbol, and range formatting providers can be invoked through standalone IPC. Advanced provider features such as completion resolve and CodeLens resolve are not modeled yet. |
| Text editor lifecycle | `window.activeTextEditor`, `TextEditor.edit`, `window.onDidChangeActiveTextEditor`, `window.onDidChangeTextEditorSelection`, `workspace.onDidChangeTextDocument` | Host-owned editor sessions provide stable editor ids, active-editor events, selection events, a document-model change event contract, basic `TextEditor.edit` insert/replace/delete application, and a bidirectional UI editable-buffer path (`editor.ui.document` -> host model -> `editor.document.changed`). No `workspace.applyEdit`, save lifecycle, decoration rendering, or multi-selection UI parity yet. |
...
| Context keys and menus | `setContext`, menu `when` filtering, `view/title`, `view/item/context` | Context keys are stored in the extension host. Static context terms are filtered before IPC; dynamic `view`/`viewItem` menu terms are filtered in the workbench. The standalone sidebar renders view-title and tree-item actions and executes them through command IPC. Other menu locations are not rendered yet, and `when` expression support is still partial. |
