...
export async function activeEs(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('airdb.elastic.document', (em: ElasticMatch) => { DocumentFinder.open(em.Path.Text) }),
        vscode.commands.registerCommand('airdb.elastic.execute', EsUtil.executeEsQueryFile),
        vscode.commands.registerCommand('airdb.elastic.lint', (em: ElasticMatch) => {
            if (em && em.HasBody) {
